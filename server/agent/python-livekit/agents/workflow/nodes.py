...
def aggregator_suggest_response(state: State):
    ...
    if DEBUG_MODE:
        print(f"\n{'='*60}")
        print(f"[WORKFLOW NODE] AGGREGATOR - INPUT DATA")
        print(f"{'='*60}")
        print(f"Card 1 Response: {json.dumps(state.get('llm_card_1_response', {}), indent=2, ensure_ascii=False)}")
        print(f"Card 2 Response: {json.dumps(state.get('llm_card_2_response', {}), indent=2, ensure_ascii=False)}")
        print(f"Card 3 Response: {json.dumps(state.get('llm_card_3_response', {}), indent=2, ensure_ascii=False)}")
